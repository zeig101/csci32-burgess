...
export default function RecipeCard({ name, description, ingredient_measurements }: RecipeCardProps) {
  return (
    <div className="border-2 border-solid border-emerald-600 rounded-md bg-slate-200 shadow-md basis-1/4 min-w-96 flex-grow">
      <Flex className="justify-between bg-slate-400 w-full p-4 gap-2 flex flex-wrap">
        <Header>{name}</Header>
      ...
      <div className="bg-slate-300 w-full p-4 flex flex-col gap-4">
        <p>{description}</p>
